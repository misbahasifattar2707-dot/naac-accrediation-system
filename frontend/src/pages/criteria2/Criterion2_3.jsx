...
const emptyRow = () => ({ passing_year: "", student_name: "", enrollment_number: "" });
...
export default function Criterion2_3() {
  ...
  const handleSave = async () => {
    for (const r of rows) {
      if (!r.passing_year || !r.student_name || !r.enrollment_number) {
        return showAlert("Please fill all fields in every row.", "danger");
      }
      const result = await addRecord("2_3", r);
    ...
    <div className="app-layout">
      ...
      <div className="main-content">
        <header className="page-header">
          ...
          <button className="btn btn-success btn-sm fw-semibold" onClick={() => navigate("/export/2-3")}>
          ...
          <div className="card border-0 shadow-sm mb-4" style={{ borderRadius: 14 }}>
            <div className="card-body p-4">
              <h6 className="fw-bold text-uppercase mb-3" style={{ fontSize: "0.78rem", letterSpacing: 1, color: "#888" }}>
                <i className="bi bi-plus-circle me-2 text-danger"></i>Add Outgoing Student Records
              </h6>
              ...
              {loadingDropdowns ? (
                ...
                <>
                  <div className="row g-2 mb-1 d-none d-md-flex">
                    {["Year of Passing", "Student Name", "Enrollment Number", ""].map((h, i) => (
                      <div key={i} className={i === 1 ? "col-md-4" : i === 3 ? "col-md-1" : "col-md-3"}>
                  ...
                  {rows.map((row, i) => (
                    <div key={i} className="row g-2 mb-2 align-items-center">
                      <div className="col-md-3">
                        <select className="form-select form-select-sm" value={row.passing_year}
                          onChange={e => updateRow(i, "passing_year", e.target.value)}>
                ...
                <tbody>
                  {records.length === 0 ? (
                    ...
                    <tr key={row.id}>
                      ...
                      <td><span className="badge" style={{ background: "#fee2e2", color: "#b31d1d", fontWeight: 700, fontSize: "0.75rem" }}>{row.passing_year}</span></td>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content border-0 shadow-lg" style={{ borderRadius: 14 }}>
              ...
              <div className="modal-body p-4">
                <div className="mb-3">
                  <label className="form-label-custom">Year of Passing</label>
                  <select className="form-select" value={editRecord.passing_year}
                    onChange={e => setEditRecord({ ...editRecord, passing_year: e.target.value })}>
                    <option value="">Select Year</option>
                    {yearOptions.map(y => <option key={y} value={y}>{y}</option>)}
                  </select>
                </div>
